...
class TableModel(FileModel, Generic[TableT]):
    ...
    @classmethod
    def _from_db(cls, path: str | PathLike[str], table: str) -> pd.DataFrame | None:
    ...
    @classmethod
    def _from_arrow(cls, path: str | PathLike[str]) -> pd.DataFrame:
...
class SpatialTableModel(TableModel[TableT], Generic[TableT]):
    ...
    @classmethod
    def _from_db(cls, path: str | PathLike[str], table: str):
        with connect(path) as connection:
            if exists(connection, table):
    ...
    def _write_table(self, path: str | PathLike[str]) -> None:
        """
        Write the contents of the input to a database.
