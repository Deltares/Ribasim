...
def laggy_reservoir_model() -> Model:
    """Create a model with a laggy reservoir, where the outflow is delayed compared to the inflow."""
    starttime = "2022-01-01"
    endtime = "2023-01-01"
    model = Model(
        starttime=starttime,
        ...
        experimental=Experimental(allocation=True),
        allocation=Allocation(timestep=86400),
        # solver = Solver(dt=86400) #seconds in day
    )
    main = model.flow_boundary.add(
    ...
    # reservoir
    rsv = model.basin.add(
        Node(2, Point(-1.0, 0.0), name="reservoir"),
        [
            basin.Profile(
                area=[5e6, 5.3e6, 5.6e6], level=[4.5, 5.5, 6.5]
            ),  # small area bc reservoir
            basin.State(level=[5.5]),
            basin.Time(time=[starttime, endtime]),
        ],
    )

    # level demand = target level
    level_d = model.level_demand.add(
        Node(3, Point(-1.0, 0.5), name="level demand"),
        [level_demand.Static(min_level=[5], max_level=[5.3], demand_priority=[1])],
    )

    # outlet
    outl = model.outlet.add(
        Node(4, Point(-2.0, 0.0), name="outlet"),
        [
            outlet.Static(
                flow_rate=[50], max_flow_rate=[9999], allocation_controlled=True
            )
        ],
    )

    end = model.terminal.add(Node(5, Point(-2.5, 0), name="end"))

    model.link.add(main, rsv, name="main")
    model.link.add(level_d, rsv, name="target level")
    model.link.add(rsv, outl, name="main2")
    model.link.add(outl, end, name="outflow")
