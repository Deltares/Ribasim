...
def discrete_control_of_pid_control_model():
    ...
    # Setup a model:
    model = ribasim.Model(
        modelname="discrete_control_of_pid_control",
        node=node,
        edge=edge,
        basin=basin,
        outlet=outlet,
...
def pid_control_connected_model():
    """Set up a model where a basin is PID controlled but also interacts with other basins."""
    ...
    xy = np.array(
        [
            ...
            (3.0, 0.0),  # 4: Pump
            (4.0, 0.0),  # 5: Basin
            (2.0, 1.0),  # 6: PidControl
        ]
    )
    node_xy = gpd.points_from_xy(x=xy[:, 0], y=xy[:, 1])

    node_type = ["Basin", "LinearResistance", "Basin", "Pump", "Basin", "PidControl"]
    ...
    state = pd.DataFrame(data={"node_id": [1, 3, 5], "level": [8.0, 2.0, 3.0]})
    ...
    pump = ribasim.Pump(
        static=pd.DataFrame(
            data={
                "node_id": [4],
                "flow_rate": [0.0],  # Overwritten by PidControl
            }
        )
    )
    ...
    # Setup a model:
    model = ribasim.Model(
        modelname="pid_control_connected",
        node=node,
        edge=edge,
        basin=basin,
        pump=pump,
