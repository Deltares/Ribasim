...
This can be connected directly to a basin and prescribes the flow to or from that basin. We adopted the convention that the edge connecting the flow boundary to the basin should point towards the basin, so that positive flow corresponds to water being added to the model.
