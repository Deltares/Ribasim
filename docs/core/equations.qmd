...
to smooth out this problem. Here $s(x) = \frac{2}{\pi}\arctan{50x}$ can be thought of as a smooth approximation of the sign function.
...
The PID controller continuously sets the flow rate of a pump or weir to bring the level of a certain basin closer to its setpoint. If we denote the setpoint by $r(t)$ and the basin level by $y(t)$, then the error is given by
$$
e(t) = r(t) - y(t).
$$ {#eq-error}
...
$$
Q_\text{pump/weir} = \begin{align}
\begin{cases}
    Q_\min & \text{if} & Q_\text{PID} < Q_\min, \\
    Q_\text{PID} & \text{if} & Q_\min \leq Q_\text{PID} \leq Q_\max, \\
    Q_\max & \text{if} & Q_\text{PID} > Q_\max.
\end{cases}
\end{align}
$$
...
$I(t)$ is treated as a state of the system and thus it has its own equation in the system in @eq-system:
$$
