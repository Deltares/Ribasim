...
# This output file is only written if the model involves control
...
- Control: Set parameters of other nodes based on model state conditions (e.g. basin level)
  - `Control / condition`: Conditions of the form 'the level in the basin with node id n is bigger than 2.0 m'
  - `Control / logic`: Translates the truth value of a set of conditions to parameter values for a controlled node
...
Control is implemented based on [VectorContinuousCallback](https://docs.sciml.ai/DiffEqDocs/stable/features/callback_functions/#VectorContinuousCallback). 
...
#### Control / condition {#sec-condition}

The control schema defines conditions of the form 'the control node with this node id listens to whether the given variable of the node with the given listen node id is grater than the given value'.

column         | type     | unit    | restriction
-------------- | -------- | ------- | -----------
node_id        | Int      | -       | sorted
listen_node_id | Int      | -       |
variable       | String   | -       | must be "level"
greater_than   | Float64  | various |

#### Control / logic

The logic schema defines which control states are triggered based on the truth of the conditions a control node listens to.
Control is applied in the Julia core as follows:

- During the simulation it is checked whether the truth of any of the conditions changes.
- When a condition changes, the corresponding control node id is retrieved (node_id in the condition schema above).
- The truth value of all the conditions this control node lisens to are retrieved, in the order as they are specified in the condition schema. This is then converted into a string of "T" for true and "F" for false. This string we call the truth state.
- The table below determines for the given control node ID and truth state what the corresponding control state is.
- For all the nodes this control node affects (as given by the "control" edges in [Edges / static](usage.qmd#edge)), their parameters are set to those parameters in `NodeType / static` corresponding to the determined control state.

column         | type     | unit | restriction
-------------- | -------- | ---- | -----------
node_id        | Int      | -    | sorted
truth_state    | String   | -    | Consists of the characters "T" (true) and "F" (false)
