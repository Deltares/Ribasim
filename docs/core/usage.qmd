...
The allocation table contains a record of allocation results: when it happened, for which node, in which allocation network, and what the demand, allocated flow and abstracted flow were.
...
For basins the values `demand`, `allocated` and `abstracted` are positive of the basin abstracts due to a shortage (e.g. its level is lower than its minimum level given by a `TargetLevel` node) and negative if the basin supplies due to a surplus of water.
