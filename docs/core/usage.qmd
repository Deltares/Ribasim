...
A linear interpolation between the given tstops is currently done if the
solver takes tstops between the given data points. More options will be available later.
...
The flow table contains results of the flow on every edge in the model, averaged over each `saveat` interval.
