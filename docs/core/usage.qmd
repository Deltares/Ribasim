...
- IntervalControl: Set parameters of other nodes based on model state conditions (e.g. basin level)
...
IntervalControl is implemented based on [VectorContinuousCallback](https://docs.sciml.ai/DiffEqDocs/stable/features/callback_functions/#VectorContinuousCallback).

#### IntervalControl / condition {#sec-condition}

The condition schema defines conditions of the form 'the interval_control node with this node id listens to whether the given variable of the node with the given listen feature id is grater than the given value'. If the condition variable comes from a time-series, a look ahead $\Delta t$ can be supplied.
...
#### IntervalControl / logic

The logic schema defines which control states are triggered based on the truth of the conditions a interval_control node listens to.
IntervalControl is applied in the Julia core as follows:
...
- The truth value of all the conditions this interval_control node lisens to are retrieved, in the order as they are specified in the condition schema. This is then converted into a string of "T" for true and "F" for false. This string we call the truth state.
- The table below determines for the given interval_control node ID and truth state what the corresponding control state is.
- For all the nodes this interval_control node affects (as given by the "control" edges in [Edges / static](usage.qmd#edge)), their parameters are set to those parameters in `NodeType / static` corresponding to the determined control state.
