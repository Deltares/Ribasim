...
- The flows $A^\text{basin}_{i}$ for all $i \in B_S$ supplied (positive) or consumed (negative) by the basins.
...
$$
    \sum_{j=1}^{n'} F_{kj} \le A^\text{basin}_k + \sum_{i=1}^{n'} F_{ik}, \quad \forall k \in B_S .
