...
The full algorithm goes thtrough the following steps:

1. Perform `internal_sources` and `collect_demands` for all subnetworks apart from the main network;
2. Perform `allocate` for the main network;
3. Perform `allocate` for the other subnetworks.

Not having a main network is also supported, then 1 and 2 are skipped.
...
$$
    C_S^p \leftarrow C_S;
$$
2. Set the the demands if they come from a timeseries,
$$
    d_j^1 \leftarrow d_j^1(t)
$$
this is skipped for `collect_demands`.
...
$$
also reduce the demands by the allocated flows and source capacities by the used flows.
...
t = 0.0
priority_idx = 1
optimization_type = Ribasim.OptimizationType.allocate

Ribasim.set_flow!(p.graph, NodeID(:FlowBoundary, 1), NodeID(:Basin, 2), 1.0)

Ribasim.set_initial_capacities_source!(allocation_model, p, optimization_type)
Ribasim.set_initial_capacities_edge!(allocation_model, p, optimization_type)
Ribasim.set_initial_capacities_basin!(allocation_model, p, u, t, optimization_type)
