...
- `collect_demands`, where flows are allocated within a subnetwork by first using the sources inside the subnetwork (internal sources), and then using main network inlet(s) as a source, with demands reduced by allocations in internal sources. The allocated flows in this optimization type are not used. The goal is to see the flow through the main network inlet(s), which is interpreted as the subnetwork demand;
