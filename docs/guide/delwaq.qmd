...
# Default parse: write concentration.nc without  modifying the model inputs
nmodel_nc = parse(model, graph, substances, to_input=False)

concentration_nc = toml_path.parent / "results" / "concentration.nc"
assert concentration_nc.is_file()
concentration_nc
```

By default (`to_input=False`), the parsed model writes the Delwaq tracer results to `results/concentration.nc`, using the same structure as the experimental Ribasim concentration output (dimensions: `node_id`, `substance`, `time`), and does not populate the `Basin / concentration_external` table.

To use the Delwaq results as inputs for plotting (or a follow-up Ribasim run), set `to_input=True` so that the `Basin / concentration_external` table is populated in addition to writing the NetCDF file.
...
# Show the first available timestep (only one may be present in the example data)
