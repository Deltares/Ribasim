...
In this guide, the relevant functionalities will be implemented in Python using the Ribasim Python package.
...
An average discharge of 44.45 $m^3/s$ is measured at the confluence.
...
To ensure that inflow equals outflow (IN=OUT) and keeping in mind the maximum depth of the river is 6 $m$, the $Q(h)$ relationship in @tbl-input2 will be used as input.

+---------------------+--------------------+
| Water Level (h) [m] | Outflow (Q) [m³/s] |
+=====================+====================+
|         0.0         |         0.0        |
+---------------------+--------------------+
|          2          |         50         |
...
- At level $2.0$: Discharge is max. $50m^3/s$. This is a bit above the average discharge rate, corresponding to the water level where normal flow conditions are established.
- At level $5.0$: Discharge rate reaches $200m^3/s$. This discharge rate occurs at the water level during wet periods, indicating higher flow capacity.
...
@fig-sim2 shows the discharges in $m^3/s$ on each edge.
...
In this case the farm company diverts from the main river an average flow rate of $10 m^3/s$ and $12 m^3/s$ during spring and summer, respectively.
...
The reservoir has a max. area of $32.3 km^2$ and a max. depth of $7 m$.
