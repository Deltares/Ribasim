...
In a conventional irrigation setup, some water is diverted from the Main River through a canal, with a portion of it eventually returning to the main river (see Figure 7).
...
Changes in this code are marked orange.
...
Schematically we are dealing with two basins. To keep the order of flow from upstream to downstream it is recommended to adjust the node_id numbers accordingly. In this case node_id = 3 will be node_id = 4 marked in orange. Basin 3 will portray as the point in the river where the diversion takes place, getting the name ‘Div’. Its profile area at this intersection is slightly smaller than at the confluence.
```python
model.basin.add(
...
The irrigated field is > 17000 ha and requires around 5mm/day.
In this case the farm company diverts from the main river an average flow rate of 10 m³/s and 12 m³/s during spring and summer, respectively.
Start of irrigation takes place on the 1st of April until the end of August.
The farmer taps water through a canal (demand).

For now, let’s assume the return flow remains 0 (return_factor).
...
Figure 9 illustrates the water levels and storage capacities for each basin.
...
Figure 12 shows the flow to the ocean (Terminal).
...
Due to the increase of population and climate change Crystal city has implemented a reservoir upstream to store water for domestic use (See Figure 13).
...
The reservoir has a max. area of 32.3 km² and a max. depth of 7 m.
...
Adjust the naming of the basin in the dictionary mapping and the saving file should be Crystal_2.1 instead of 1.2.
...
Figure 14 illustrates the new storage and water level at the reservoir.
...
Figure 15 shows the flow to (PWS Demand) and out (PWS Return) of the PWS node.
Figure 16 shows the downstream flow to the ocean.
The impact is clear.
