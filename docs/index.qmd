...
In Ribasim, we make use of DifferentialEquations.jl and can use any of its [ODE solvers](https://diffeq.sciml.ai/stable/solvers/ode_solve/).
...
Binaries of `ribasim_cli` can be downloaded from
...
The `ribasim_cli` executable is in the `bin` directory.

To check whether the installation was performed successfully, run `ribasim_cli` with no
arguments in the command line will give the following message:

```
Usage: ribasim_cli 'path/to/config.toml'
```
...
and add the Ribasim package to your project environment. Note that to receive updates, you
...
- Use `ribasim_cli` if you only need `Ribasim.run` and don't need to do developments.
- Adapt a workflow that keeps you session active; the first `Ribasim.run("model.toml")` will
...
# run a coupled Ribasim - Modflow simulation
...
network_path = "input/network.ply"
...
languages. In Ribasim we use [Arrow.jl](https://arrow.juliadata.org/dev/).
...
Currently the only states are the volume and salinity of storage.

column    | type    | restriction
--------- | ------- | -----------
location  | Int     | -
volume    | Float64 | non-negative
salinity  | Float64 | non-negative
...
From [this link](https://github.com/visr/ribasim-artifacts/releases/download/v0.1.0/schematization.zip)
