...
The new node type might have associated restrictions for a model with the new node type so that it behaves properly. Basic node ID and node type validation happens in `Model.validate_model` in `python/ribasim/ribasim/model.py`, which automatically considers all node types in the `node_types` module. Connectivity validation happens in `is_valid` in `core/src/solve.jl`. Connectivity rules are specified by stating the allowed downstread node types for `NewNodeType` in `core/src/validation.jl`:
...
Future: Test for the specific behaviour of each node type
