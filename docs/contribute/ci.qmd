...
* Build Ribasim: builds library of Ribasim on Linux and Windows. The artifact is tested in `Test Ribasim Binaries` and used by iMOD Coupler. And it builds CLI application on Linux and Windows, its artifact is tested in `Test Ribasim Binaries` and used by release
...
graph LR
    ...
    H[Build libribasim]---D[Test Ribasim Binaries]
    D-->A
```
