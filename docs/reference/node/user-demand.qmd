---
title: "![](/assets/node-icons/UserDemand.svg){height=20} UserDemand"
---
...
--------------- | ------- | ----------------------| -----------
node_id         | Int32   | -                     |
demand          | Float64 | $\text{m}^3/\text{s}$ | non-negative (optional)
...
values are block interpolated (forward fill), and outside the demand is constant given by the
...
The actual (supplied) abstraction rate of a UserDemand is given by
$$
    Q_\text{in} = \phi(u, S_{wd0.1})\phi(h-l_{\min}, 0.02)\sum_{p=1}^{p_{\max}} \min\left(F^p, d^p(t)\right).
$$

From left to right:

- The first reduction factor lets the UserDemand abstraction go smoothly to $0$ as the source Basin dries out below a storage which is the equivalent of a water depth of $10 \;\text{cm}$ (change with solver setting `depth_threshold`);
- The second reduction factor lets the UserDemand abstraction go smoothly to $0$ as the source Basin level approaches the minimum source Basin level (from above) (change with solver setting `level_difference_threshold`);
