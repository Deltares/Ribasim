---
title: "![](/assets/node-icons/Basin.svg){height=20} Basin"
---
...
Examples include a river or canal section, lake, or reservoir.
...
---------             | ------- | --------------------- | -----------
node_id               | Int32   | -                     |
drainage              | Float64 | $\text{m}^3/\text{s}$ | non-negative (optional)
potential_evaporation | Float64 | $\text{m}/\text{s}$   | non-negative (optional)
infiltration          | Float64 | $\text{m}^3/\text{s}$ | non-negative (optional)
precipitation         | Float64 | $\text{m}/\text{s}$   | non-negative (optional)
surface_runoff        | Float64 | $\text{m}^3/\text{s}$ | non-negative (optional)
...
To use the final state of an earlier simulation as an initial condition, copy [`results/basin_state.nc`](/reference/usage.qmd#sec-state) over to the `input_dir`, and point the TOML to it:

```toml
[basin]
state = "basin_state.nc"
```
...
In between the given times the values are block interpolated (forward fill), and outside the concentration is constant given by the nearest time value.
...
## Mass load
This table is used for dry (waste) loads, i.e. mass injection into a basin.
