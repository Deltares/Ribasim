...
--------- | ------- | ------------ | -----------
node_id   | Int32   | -            |
storage   | Float64 | $\text{m}^3$ | non-negative and non-decreasing
area      | Float64 | $\text{m}^2$ | non-negative, per node_id: start positive
...
Internally this get converted to two functions, $A(S)$ and $h(S)$.
When $A$ is missing, we take the derivative $S$ with respect to $h$.
...
ax.plot([level[-1], level_extrap], [area[-1], area[-1]], color = "C0", ls = "dashed", label = "extrapolation")
xlim = ax.get_xlim()
