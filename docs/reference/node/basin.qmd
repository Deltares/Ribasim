...
The Basin node represents a flexible, generalized control volume that can be used to model various water bodies and systems, including reservoirs, lakes, river reaches, and canals.
While the term "Basin" may traditionally suggest a watershed, in this context, it serves as an abstraction that can represent a wide range of water bodies and control volumes, including those that may not conform to the typical geographical definition of a basin.

The Basin node is the central node in each schematization, as it is the only one that stores water.
...
# Using the Basin Node for Different Water Bodies
...
Lakes: Similar to reservoirs, lakes can be modeled by defining the area and water level in the profile table, while the static and time tables are used to define other parameters like precipitation and evaporation.

River Reaches: For river reaches, the Basin node can represent a segment of the river. The profile table is used to define the cross-sectional area and hydraulic properties, while the time and static tables capture inflows and other dynamic behaviors.

Canals: A canal can also be modeled as a Basin, where water flow is managed along the canal system. The profile table defines the cross-sectional area, and the static or time tables are used for dynamic inflows or other relevant parameters.

# Confluence Representation
A Confluence where multiple links converge into a single node are also modeled using the Basin node.
Since Ribasim does not have a dedicated confluence node yet, the Basin node is used to represent the combined flow at the point where multiple water sources converge.

To model a confluence, you would define the appropriate profile table for the Basin, specifying parameters such as area and water level. The flow behavior at the confluence will depend on the incoming flows from the connected nodes and how they interact at the confluence point.

In this way, the Basin node serves as a flexible tool for modeling complex systems where multiple water sources come together, ensuring that water is managed and stored appropriately at these points of convergence.
