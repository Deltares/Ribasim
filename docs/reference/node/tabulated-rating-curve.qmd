...
For tabulated rating curves with a fixed maximum value (e.g. max capacity of a discharge gate), enter a new row and re-enter the maximum flow_rate at an incrementally higher level:
...
Ribasim does not allow the flow rate to decrease at higher levels. To do so, a DiscreteControl node is needed to manipulate the flow_rate. 
