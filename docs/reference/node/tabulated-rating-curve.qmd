...
To enable this, set `allocation_controlled = True` in the Static table.
...
$$
Q = \phi \cdot \min(Q_\text{allocated},\; Q(h))
$$
...
When experimental allocation is disabled, `allocation_controlled` has no effect and the TabulatedRatingCurve follows its normal physics-based $Q(h)$ relationship.
