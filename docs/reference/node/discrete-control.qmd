...
interval             | Float64  | $\text{s}$ | (optional), positive, must be the same for all rows of a node_id
...
By default the control logic of a DiscreteControl node is applied after every solver timestep. With `interval` it is instead applied only at multiples of that interval since the simulation start time, which is often a more realistic representation of human operation. It also bounds how often a control state can change, which helps for models where conditions would otherwise flip on nearly every timestep. Since the interval is a property of the DiscreteControl node rather than of an individual condition, it must be the same for all rows with the same `node_id`.
