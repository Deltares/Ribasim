...
::: {.callout-note}
It is possible to configure a DiscreteControl node such that it very rapidly oscillates between control states. To make you aware of this, Ribasim will throw an error if successive control state changes of a DiscriteControl node in simulated time are closer than `minimal_discrete_control_update_dt`, which is configurable.
:::
