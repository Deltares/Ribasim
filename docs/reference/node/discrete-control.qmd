...
The condition schema defines conditions of the form 'the `discrete_control` node with this `node_id` listens to whether the variable given by the `node_id` and `compound_variable_id` is greater than `greater_than`' (at time $t$) when the condition was false previously, or `less_than` when true previously. Using `less_than` is optional and defaults to `greater_than`. When set to a different value---smaller than or equal to `greater_than`---it defines a [deadband]("https://en.wikipedia.org/wiki/Deadband") and thus enables hysteresis. In equation form:
...
Multiple conditions with different `greater_than` and/or `less_than` values can be defined on the same compound_variable, each with their own unique `condition_id`.

Note the strict inequality '$>$' in the equation above. This means for instance that if a simulation starts with a compound variable exactly at the `greater_than` value, the condition is false. The same holds for the `less_than` value, using '$>=$', so the deadband is *inclusive* of the thresholds.
...
less_than            | Float64  | various | (optional), <= greater_than
