...
- The flow through one of these node types: Pump, Outlet, TabulatedRatingCurve, LinearResistance, ManningResistance
...
Note the strict inequality '$>$' in the equation above. This means for instance that if a simulation starts with a comound variable exactly at the `greater_than` value, the condition is false.
