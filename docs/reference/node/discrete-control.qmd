...
\text{condition}=\begin{cases}
    \text{weight}_1 * \text{variable}_1 + \text{weight}_2 * \text{variable}_2 + \ldots > \text{greater_than}(t), !\text{condition_prev} \\
    \text{weight}_1 * \text{variable}_1 + \text{weight}_2 * \text{variable}_2 + \ldots >= \text{less_than}(t), \text{condition_prev}
\end{cases}
$$
...
To make you aware of these oscillations, Ribasim will throw an error if successive control state changes of a DiscriteControl node in simulated time are closer than `minimal_discrete_control_update_dt`, which is configurable.
