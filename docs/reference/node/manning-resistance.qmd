...
And $x_0$ is calculated based on the Reynolds number, such that the smooth relaxation starts when the velocity of the flow starts to leave the fully turbulent regime:

$$
x_0 = \frac{\Delta h}{L} |_0 = \left( \text{Re}_0 \frac{ n \nu \sqrt[3]{R_h}}{A} \right)^2
$$

And the transition Reynolds number, with a value of 2000 is defined for open channel flow as:
$$
\text{Re} = \frac{Q}{\nu R_h}
$$
...
ax.plot(x, y_s, color = "C0", label = r"$s\left(x; 1e-5 \right)$")
ax.legend();
```
