---
title: "![](https://s3.deltares.nl/ribasim/doc-image/node-icons/png/LinearResistance.png){width=30} LinearResistance"
---
...
max_flow_rate | Float64 | $\text{m}^3/s$        | non-negative (optional)
...
$\phi$ is the reduction factor which makes the flow go smoothly to $0$ as the upstream storage (as determined by the flow direction) becomes smaller than the equivalent of a water depth of $10 \;\text{cm}$ by default (change with solver setting `depth_threshold`).
