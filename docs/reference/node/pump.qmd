...
When [PID controlled](/reference/node/pid-control.qmd), the pump must point away from the controlled basin in terms of links.
...
With this all parameters can be updated over time. In between the given times
constant interpolation (forward fill) is used, and outside the given time range the values are kept constant at the
