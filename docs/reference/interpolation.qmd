...
# Interpolation
ax.set_title("level → area", fontsize = fontsize_title)
...
ax.set_xticks([])
ax.set_yticks([0])
ax.set_title("level → storage", fontsize = fontsize_title)
ax.set_xlabel("level", fontsize = fontsize)
...
def plot_forward_fill(i):
    ax.plot([x[i], x[i+1]], [y[i], y[i]], color = "C0")

ax.scatter(x,y, label = "forcing at data points")
for i in range(N-1):
    plot_forward_fill(i)

x_missing_data = np.random.rand(3)
