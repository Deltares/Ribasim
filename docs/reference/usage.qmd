...
3. "listen": The listen links define which nodes are listened to by control nodes.
  ...
  Listen links can also be added manually with `model.link.add(from_node, to_node)`, where `from_node` is a listenable node (e.g. a Basin) and `to_node` is a control node (e.g. DiscreteControl or PidControl). The link type is automatically inferred as `"listen"` from the node types.
