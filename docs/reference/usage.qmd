...
Finally there's `specialize = false`. By default it is turned off to reduce the time it takes to initialize fully. It can be enabled for long-running simulations, trading initialization speed for simulation speed. Concretely, enabling it will set the [specialization level](https://docs.sciml.ai/SciMLBase/stable/interfaces/Problems/#Specialization-Levels) to NoSpecialize for false, and FullSpecialize for true. Additionally setting it to true also fixes the [autodiff chunk size](https://juliadiff.org/ForwardDiff.jl/stable/user/advanced/#Configuring-Chunk-Size) to 1, making a similar tradeoff as the specialization level.
