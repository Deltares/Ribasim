...
variables:
  Float64   level(time, node)          // m
  Float64   storage(time, node)        // m³
  Float64   inflow_rate(time, node)    // m³/s
  Float64   outflow_rate(time, node)   // m³/s
  Float64   storage_rate(time, node)   // m³/s
  Float64   precipitation(time, node)  // m³/s
  Float64   surface_runoff(time, node) // m³/s
  Float64   evaporation(time, node)    // m³/s
  Float64   drainage(time, node)       // m³/s
  Float64   infiltration(time, node)   // m³/s
  Float64   balance_error(time, node)  // m³/s
  Float64   relative_error(time, node) // dimensionless
  Float64   convergence(time, node)    // dimensionless
```
...
variables:
  String    from_node_type(link)     // Node type
  Int32     from_node_id(link)       // Node ID
  String    to_node_type(link)       // Node type
  Int32     to_node_id(link)         // Node ID
  Float64   flow_rate(time, link)    // m³/s
  Float64   convergence(time, link)  // dimensionless
```
...
```cdl
dimensions:
  time   = <number of control changes>

coordinates:
  DateTime  time(time)

variables:
  Int32     control_node_id(time)  // Node ID
  String    truth_state(time)      // Truth state
  String    control_state(time)    // Control state
```
...
```cdl
dimensions:
  time   = <number of allocation timesteps>
  node   = <number of allocated nodes>

coordinates:
  DateTime  time(time)

variables:
  Int32     subnetwork_id(time, node)   // Subnetwork ID
  String    node_type(time, node)       // Node type
  Int32     node_id(time, node)         // Node ID
  Int32     demand_priority(time, node) // Priority
  Float64   demand(time, node)          // m³/s
  Float64   allocated(time, node)       // m³/s
  Float64   realized(time, node)        // m³/s
```
...
dimensions:
  time   = <number of allocation timesteps>
  link   = <number of allocation links>
...
variables:
  ...
  String    to_node_type(link)         // Node type
  Int32     to_node_id(link)           // Node ID
  Int32     subnetwork_id(link)        // Subnetwork ID
  Float64   flow_rate(time, link)      // m³/s
  Bool      upper_bound_hit(time, link)// Boolean
  Bool      lower_bound_hit(time, link)// Boolean
```
...
dimensions:
  time       = <number of timesteps>
  node       = <number of Basin nodes>
  substance  = <number of substances>
...
variables:
  Float64   concentration(time, node, substance)  // g/m³
```
...
dimensions:
  time     = <number of timesteps>
  subgrid  = <number of subgrid elements>

coordinates:
  DateTime  time(time)
  Int32     subgrid_id(subgrid)

variables:
  Float64   subgrid_level(time, subgrid)  // m
```
...
variables:
  Float64   computation_time(time)    // ms
  Int       rhs_calls(time)           // count
  Int       linear_solves(time)       // count
  Int       accepted_timesteps(time)  // count
  Int       rejected_timesteps(time)  // count
  Float64   dt(time)                  // s
```
