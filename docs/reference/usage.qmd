...
`depth_threshold = 0.1` is the water depth (level - profile bottom) in meters at which the low storage factor kicks in. This will limit the evaporation and infiltration from nearly empty Basins.
`level_difference_threshold = 0.02` is the level difference below which several flows are reduced. Examples are approaching `min_upstream_level` from above and `max_downstream_level` from below, level difference across TabulatedRatingCurve and Outlet nodes, and UserDemand approaching the `min_level` from above. For details see the equations on the node [reference pages](/reference/index.qmd)
