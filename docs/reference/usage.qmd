...
When a node is to be controlled by allocation, use a special control state name. In the Static table, set `control_state = "allocation"` to indicate that this node's flow rate can be dynamically adjusted by the allocation algorithm to achieve optimal water distribution.
