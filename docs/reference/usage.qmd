...
If your model does not converge, or your performance is lower than expected, it can help to adjust other solver settings as well. For example, setting `reduced_implicit_solve = false` will bypass much of the custom numerical code written for Ribasim, using a less optimized but potentially more robust algorithm.
...
By default (`reduced_implicit_solve = true`) Ribasim supplies its own Jacobian, which is never formed as a matrix.
...
This makes the `autodiff` and `sparse` settings below only relevant for the fallback `reduced_implicit_solve = false`, with the exception that `sparse` also selects the factorization used in the linear solve of both.
...
This is the case when a fixed timestep (`dt`) is combined with an explicit algorithm, since then there is neither a nonlinear solve nor an error estimate, and when `reduced_implicit_solve = false`.
