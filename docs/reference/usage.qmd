...
- `source_priority`: An integer per source type for the allocation algorithm. The prioritisable sources are: `basin`, `level_boundary`, `linear_resistance`, `manning_resistance`, `outlet` and `pump`.
  - If you wish to set the source priority for specific nodes rather than a fallback per node type, these can be set in the [Node table](#sec-node).
  - By default, all nodes of the same type have the same source priority. To obtain a strict source ordering, the sources are sorted by node ID for each source priority within a subnetwork.
  - When no default source priorities are specified, default values are applied (see the TOML example above).

  The way the source priorities work is as follows: The priority that is assigned to a node is interpreted as the cost it takes for water to flow through that node. The flow through each node is multiplied with the priority, and we find the water distribution solution that has the lowest cost of fulfilling the desired demands.
...
----------------- | ------ | -----------
format            | String | File format, default is "arrow" for Apache Arrow, other option is "netcdf" for NetCDF with CF conventions.
...
If not set, the `source_priority` is fixed per node type as explained in the [allocation settings](#sec-allocation-settings).
