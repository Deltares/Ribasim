...
Ribasim expects the GeoPackage database [database.gpkg](/reference/usage.qmd#sec-geopackage) as well as optional NetCDF input files to be available in the input_dir.
...
If your model does not converge, or your performance is lower than expected, it can help to adjust other solver settings as well. For example, setting `optimized_implicit_solve = false` will bypass much of the custom numerical code written for Ribasim, using a less optimized but potentially more robust algorithm.
...
By default (`optimized_implicit_solve = true`) Ribasim supplies its own Jacobian, which is never formed as a matrix.
Its structure follows directly from the network, and the blocks that cannot be written down analytically are obtained with [ForwardDiff.jl](https://juliadiff.org/ForwardDiff.jl/stable/) through [DifferentiationInterface.jl](https://juliadiff.org/DifferentiationInterface.jl/DifferentiationInterface/stable/).
This makes the `autodiff` and `sparse` settings below only relevant for the fallback `optimized_implicit_solve = false`, with the exception that `sparse` also selects the factorization used in the linear solve of both.

In the fallback, the full Jacobian of the right hand side is computed with [DifferentiationInterface.jl](https://juliadiff.org/DifferentiationInterface.jl/DifferentiationInterface/stable/).
By default this uses ForwardDiff.jl; setting `autodiff = false` switches to a finite difference method, which can be less accurate and more expensive.

Also in the fallback, the Jacobian is a sparse matrix by default (`sparse = true`).
...
The sparsity structure is detected with [SparseConnectivityTracer.jl](https://adrianhill.de/SparseConnectivityTracer.jl/stable/) and provided as a Jacobian prototype to the solver.
...
By default `specialize = true`, which sets the [specialization level](https://docs.sciml.ai/SciMLBase/stable/interfaces/Problems/#Specialization-Levels) to FullSpecialize — the fastest option for Ribasim on both initialization and simulation. Setting `specialize = false` (NoSpecialize) is not recommended and may be removed in a future release.
...
- `dt`: A float value in seconds which sets a fixed update interval for allocation. If not provided (the default), adaptive timestepping is used, where the interval is determined automatically based on linearization error bounds (see [Adaptive timestepping](/concept/allocation.qmd#sec-adaptive-timestepping));
- `dtmin`: The minimum allocation timestep in seconds when using adaptive timestepping (default: 3600.0);
- `reltol_linearization`: Relative tolerance for linearization errors of basin profiles and connector node Q(h) relationships when using adaptive timestepping (default: 0.01);
...
----------------- | ------ | -----------
compression       | Bool   | Whether to apply deflate compression or not.
compression_level | Int    | Deflate compression level (0-9). Default is 1, higher compresses more.
...
# GeoPackage database and NetCDF tables {#sec-geopackage}
...
results are always written to NetCDF files. All other tables can either be in the database or in separate
NetCDF files that are listed in the TOML as described above.
...
NetCDF is the format used for results and external input tables. NetCDF
...
## Using NetCDF for input tables {#sec-netcdf-input}

By default, all tables are stored in the GeoPackage database.
However, you can manually configure to store and read data in external files.
This is particularly useful when you quickly want to be able to switch between different time series on the same model.


### Writing tables to NetCDF with Python
...
# Write the model - this creates the TOML, database, and NetCDF file
...
```python
# Store profile data back in the GeoPackage
...
When Pumps and Outlets are part of a subnetwork, they can be controlled by allocation. To accomplish this, besides a subnetwork_id, they must have `allocation_controlled` set to `true` in the static table for [Pump](/reference/node/pump.qmd#static) or [Outlet](/reference/node/outlet.qmd#static).

# Link {#sec-link}

Links define connections between nodes. The only thing that defines a link is the nodes it connects, and in what direction.
There are currently 4 possible link types:
...
from_node_id  | Int32                         | -
to_node_id    | Int32                         | -
link_type     | String                        | must be "flow", "control", "listen", or "observation"
...
Results are written in NetCDF format by default, which stores data as multidimensional arrays rather than flat tables.
This section describes the structure of each NetCDF result file using CDL-like notation.

```{python}
#| include: false
%run ../run_ribasim.py

# Run models used for results section
from pathlib import Path

models = Path("../../generated_testmodels")
run_ribasim(models / "basic/ribasim.toml")
run_ribasim(models / "pump_discrete_control/ribasim.toml")
run_ribasim(models / "flow_demand/ribasim.toml")
run_ribasim(models / "two_basin/ribasim.toml")
```
...
- Results of the fluxes on each Basin, which are mean values over the `saveat` intervals.
  In the time coordinate the start of the period is indicated.
...
- The total inflow is `inflow_rate` (horizontal) + `precipitation` + `surface_runoff` + `drainage`.
- The total outflow is `outflow_rate` (horizontal) + `evaporation` + `infiltration`.
- The `balance_error` is the difference between the `storage_rate` on one side and the total inflow and outflow on the other side: `storage_rate` - (total inflow - total outflow).
...
- The `convergence` indicates how hard this Basin was to solve compared to the other Basins, see [convergence output](#sec-convergence).
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/basic/results/basin.nc")
ds
```

## Flow - `flow.nc`

The flow results contain calculated mean flows over the `saveat` intervals for every flow link in the model.
In the time coordinate the start of the period is indicated.
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/basic/results/flow.nc")
ds
```
...
The `convergence` indicates how hard the flow over this link was to solve compared to the other flows, see [convergence output](#sec-convergence).

## Convergence output {#sec-convergence}

Both the Basin and the flow results contain a `convergence` column, which indicates which parts of the model are hardest for the solver to solve.
This is useful to find the bottleneck when a simulation is slow or fails to converge.
The same information is written to the log at the end of a simulation, as the water balance bottlenecks (Basins) and the flow physics bottlenecks (flow links).

While taking the timesteps within a `saveat` interval, the solver repeatedly evaluates the scaled residual of every state.
How these residuals are scaled is described in the [numerical considerations](/concept/numerics.qmd).
Each time this happens, the residuals of the Basin storages are scaled such that the largest one becomes 1, and the same is done separately for the residuals of the flows.
The reported `convergence` is the average of these scaled values over all residual evaluations in the `saveat` interval, and therefore always lies between 0 and 1.

This means that `convergence` is a relative measure, and not an absolute error:

- A value close to 1 means that this Basin or link was consistently the hardest to solve, and is a likely candidate for causing small timesteps.
- A value close to 0 means that this Basin or link never limited the solver.
- Because the value is relative, every simulation has states with a high `convergence`, also when the model solves without any problems.

The `convergence` is `missing` if the solver did not evaluate any scaled residuals in the `saveat` interval.
This is the case when a fixed timestep (`dt`) is combined with an explicit algorithm, since then there is neither a nonlinear solve nor an error estimate, and when `optimized_implicit_solve = false`.

## State - `basin_state.nc` {#sec-state}
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/basic/results/basin_state.nc")
ds
```
...
## DiscreteControl - `control.nc`

The control results contain a record of each change of control state: when it happened, which control node was involved, to which control state it changed and based on which truth state.
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/pump_discrete_control/results/control.nc")
ds
```

## Allocation - `allocation.nc`

The allocation results contain a record of allocation results: when it happened, for which node, in which allocation network, and what the demand, allocated flow and supplied flow were.
The supplied values at the starting time of the simulation can be ignored.

**NetCDF structure:**

```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/flow_demand/results/allocation.nc")
ds
```
...
For Basins the values `demand`, `allocated` and `supplied` are positive if the Basin level is below the minimum level given by a `LevelDemand` node.
...
## Allocation flow - `allocation_flow.nc`

The allocation flow results contain results of the optimized allocation flow on every link in the model that is part of a subnetwork, for each time an optimization problem is solved (see also [here](/concept/allocation.qmd#the-high-level-algorithm)).
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/flow_demand/results/allocation_flow.nc")
ds
```
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/basic/results/concentration.nc")
ds
```

::: {.callout-note}
Only Basin nodes are included in the concentration output.
Substances can correspond to known Delwaq substances.
:::

## Subgrid level - `subgrid_level.nc`
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/two_basin/results/subgrid_level.nc")
ds
```

## Solver statistics - `solver_stats.nc`

This result file contains statistics about the solver, which can give an insight into how well the solver is performing over time. The data is saved by `saveat` (see [configuration file](#configuration-file)). `water_balance` refers to the right-hand-side function of the system of differential equations solved by the Ribasim core.
...
```{python}
#| code-fold: true
ds = xr.open_dataset("../../generated_testmodels/basic/results/solver_stats.nc")
ds
```
