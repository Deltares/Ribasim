...
# Specify that the profile table should be written to Arrow format
...
- `my_model/input/profile.arrow` - Arrow file with Basin profile data
...
You can also use this to write time series to NetCDF:
```python
# Add time-varying data
model.basin.time.filepath = Path("basin-time.nc")
```

**Note**: Only tables with data will have their filepath written to the TOML. If you set a
filepath on an empty table, it will be ignored to prevent errors.
