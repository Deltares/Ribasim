...
When the model is found to be unstable, `max_depth` gives an upper bound for the Basin depth above which the Basin will be flagged as behaving implausibly.
...
- `dtmax`: The maximum allocation timestep in seconds for adaptive timestepping, typically the human intervention interval (defaults: 86400.0);
...
If not set, `route_priority` defaults to the value configured for this node type in the `route_priority` section of the config; see the [allocation settings](#sec-allocation-settings).
...
Pumps and Outlets that connect the primary network to a secondary network must have `allocation_controlled` set to `true`; an error is thrown otherwise.
...
- The `convergence` is a severity score for this node: the share of the local error estimate attributed to it, averaged over the `saveat` interval and normalized such that the worst node of every timestep scores 1. It ranks the nodes by how much they hold the solver back, so a high value does not mean the node is wrong; it means it is a bottleneck, and therefore a good candidate for reparameterization when the model is slow. See [debugging slow models](/guide/debug.qmd#slow-models).
...
- The `convergence` is a severity score for this link: the share of the local error estimate attributed to it, averaged over the `saveat` interval and normalized such that the worst state of every timestep scores 1. A high value does not mean the link is wrong; it means it is a bottleneck for the solver. See [debugging slow models](/guide/debug.qmd#slow-models).
...
The `dt` is the average size (in seconds) of the calculation timesteps in the given period.

A timestep that is rejected is thrown away and retried with a smaller step, so rejections are pure overhead.
`rejected_timesteps` is split out by cause:

- `rejected_local_error`: the estimated error over the step was too large. This is the signature of a stiff model, where some node reacts much faster than the rest.
- `rejected_nonlinear_solve`: the solver could not solve the equations to the required accuracy.
- `rejected_out_of_domain`: the step left the physically valid domain. Currently this means it would have given a Basin a negative storage.

The `mean_order` is the average order of the accepted timesteps in the given period.
The default solver adapts its order up to 5; a value that stays near 1 or 2 indicates a stiff model or a discontinuity that keeps resetting the solver.

See [debugging slow models](/guide/debug.qmd#slow-models) for how to use these.
