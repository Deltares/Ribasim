...
- `default_route_priority`: An integer per source type for the allocation algorithm. The prioritisable sources are: `basin`, `level_boundary`, `linear_resistance`, `manning_resistance`, `tabulated_rating_curve`, `outlet` and `pump`.
...
If not set, `route_priority` defaults to the value configured for this node type in `default_route_priority`; see the [allocation settings](#sec-allocation-settings).
