...
## Subnetwork ID
Nodes can only be controlled by allocation if the have a subnetwork id. In the Node table, give for example subnetwork_id and integer value

## Control by allocation

When a node is to be controlled by allocation, use a special control state name. In the Static table, set `control_state = "Ribasim.allocation"` to indicate that this node's flow rate can be dynamically adjusted by the allocation algorithm to influence water distribution.
