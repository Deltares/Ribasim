...
Currently compression is only used for Apache Arrow.
NetCDF will be the only output format later on.
...
[Arrow.jl](https://arrow.apache.org/julia/dev/). Results can be written to Arrow, since for long
...
NetCDF is also supported as an alternative format for input tables. NetCDF files follow the
[CF conventions](http://cfconventions.org/) and are particularly useful for large time series
data or when working with tools that prefer the NetCDF format. However, note that NetCDF input
tables are currently [not automatically loaded in QGIS](/known_issues.qmd).

## Using Arrow or NetCDF for input tables {#sec-arrow-netcdf-input}

By default, all tables except Node and Link are stored in the GeoPackage database.
However, for large tables (e.g., time series data), it can be more efficient to store them
in separate Arrow or NetCDF files. This is particularly useful for:

- Large time series that would make the database file unwieldy
- Tables that need to be easily exchanged between different models
- Performance optimization for very large datasets

### Writing tables to Arrow or NetCDF with Python
...
# Write Basin profile table to Arrow format
model.basin.profile.filepath = Path("profile.arrow")

# Write Basin time series to NetCDF format
model.basin.time.filepath = Path("basin-time.nc")

# The files will be created when you write the model
model.write("path/to/model.toml")
```

The file format is determined automatically from the file extension:
- `.arrow` for Apache Arrow format
...
For example, the TOML might look like:
```toml
[basin]
...
### Using external table files in QGIS

**Important limitation**: Tables stored in Arrow or NetCDF files are currently not automatically
loaded in the QGIS plugin (see [#318](https://github.com/Deltares/Ribasim/issues/318) and
[#2519](https://github.com/Deltares/Ribasim/issues/2519)). If you need to edit tables
manually using QGIS, keep them in the GeoPackage database. Arrow and NetCDF formats are
best suited for:

- Large models where manual editing is not practical
- Programmatically generated models
- Performance-critical applications
- Scenarios where you primarily interact with the model via Python or the CLI
