...
### Added
- Add `rejected_nonlinear_solve`, `rejected_local_error` and `rejected_out_of_domain` to `results/solver_stats.nc`, which split the rejected timesteps by cause.
- Add `mean_order` to `results/solver_stats.nc`, the average order of the accepted timesteps.
- Python: `solver.min_discrete_control_interval` is now settable; it was documented but missing from the `Solver` class.

### Changed
- `rejected_timesteps` in `results/solver_stats.nc` now also counts timesteps that were rejected because the nonlinear solver did not converge, so it is the sum of the three causes above. These were previously left out, making it report fewer rejections than actually took place.
- Convergence bottlenecks are now also logged for models that finish successfully, not only for failed or interrupted runs.
- The `dt` in `results/solver_stats.nc` is now the average timestep size over the `saveat` interval, instead of the size of the last timestep.
- Allocation: Allocation controlled Pump and Outlet nodes now require a finite `max_flow_rate`. Models that do not satisfy this are rejected with an error.
- Allocation: Pump and Outlet nodes that connect the primary network to a secondary network are no longer made allocation controlled automatically; they must have `allocation_controlled` set to `true`, otherwise an error is thrown.

### Fixed
- A DiscreteControl state change is now reported to the integrator as a discontinuity of the right hand side. It previously changed the parameters silently, so the integrator carried a derivative that no longer matched the equations, which showed up as rejected timesteps.
- The initial DiscreteControl state is now based on the initial value of each condition, instead of always starting from `false`. A condition with hysteresis that starts between `threshold_low` and `threshold_high` now takes the side it is closest to, rather than always starting below both and having to cross `threshold_high` before it responds. Models without hysteresis are unaffected.
- The nonlinear solver now scales its increment by the change of each state over the timestep, like the error estimate does. It previously fell back to scaling by the cumulative magnitude of the states, which made its convergence check increasingly lenient over a simulation.
- The first timestep is now exempt from the `solver.min_discrete_control_interval` check. Setting the initial control state at $t = 0$ no longer counts as a control state change, which made any change on the first timestep fail.
- Allocation: Normalize LevelDemand fairness constraints by the current mean Basin area per demand priority, improving numerical conditioning for large Basins.
- Allocation: Use a deterministic minimum-inlet-flow tie-break when collecting secondary-network demands from the primary network.
