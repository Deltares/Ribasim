...
When Pumps, Outlets, and TabulatedRatingCurves are part of a subnetwork, they can be controlled by allocation. To accomplish this, they must be given the special control state `Ribasim.allocation` in the static table for [Pump](/reference/node/pump.qmd#static), [Outlet](/reference/node/outlet.qmd#static), or [TabulatedRatingCurve](/reference/node/tabulated-rating-curve.qmd#static). After all goals have been optimized for, the flow rate through the node is communicated to the physical layer.

For **Pumps and Outlets**, the flow through the node is only bounded by its capacity and node-specific constraints based on the difference between upstream and downstream level (see [Pump equations](/reference/node/pump.qmd#equations) or [Outlet equations](/reference/node/outlet.qmd#equations)).

For **TabulatedRatingCurves**, the allocation-determined flow is bounded by the maximum flow the rating curve can physically deliver at the current upstream level, i.e. $Q(h)$ evaluated at the expected level at the end of the allocation time step. In the physical layer, the allocated flow rate replaces the usual $Q(h)$ lookup while still applying the same reduction factors for low storage, level difference, and maximum downstream level (see [TabulatedRatingCurve equations](/reference/node/tabulated-rating-curve.qmd#equations)).
