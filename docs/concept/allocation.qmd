...
The allocation problem is solved separately for each subnetwork in the Ribasim model. Subnetworks are defined by grouping nodes that share the same subnetwork ID in the [Node table](../reference/usage.qmd#node).

The network can include:

- A primary network (subnetwork ID 1) representing the main water system
- Multiple secondary networks which:
    - Can only connect to the primary network via Pump or Outlet nodes
    - Cannot connect directly to other secondary networks
    - Automatically formulate demands to the primary network when they have insufficient water to meet internal demands

For each subnetwork, a linear optimization problem is formulated using [JuMP](https://jump.dev/JuMP.jl/stable/) and solved with the [HiGHS solver](https://highs.dev/).
...
$$
\begin{aligned}
\text{min}\quad & z = \sum_{i}^{N_d} E_i(Q, S) \\
\text{s.t.}\quad & \frac{dS_i}{dt} = Q_{i,in} - Q_{i,out}\\
\end{aligned}
$$
...
- $S$ is the storage of a basin [m³]
- $t$ is the time [s]
- $N_d$ is the number of demand nodes [-]
- $E_i$ is an error as function of flow and storage
...
Using backward Euler approximation:

$$\frac{dS}{dt} \approx \frac{S^{n+1} - S^{n}}{\Delta t} = Q_{in}^{n+1} - Q_{out}^{n+1} $$

Where $n$ denotes evaluation at the beginning and $n+1$ at the end of the timestep.

The allocation timestep $\Delta t$ must be:

1. Short enough that linearization remains valid
2. Long enough for meaningful allocation decisions
3. Compatible with the physical timestep

## Linearization

To use efficient linear programming algorithms, nonlinear relationships are linearized around the current operating point by:

1. Evaluating quantities at allocation timestep start
2. Using first-order Taylor series expansions
3. Converting nonlinear constraints to linear approximations

For example, a nonlinear flow-storage relationship $Q = f(S)$ becomes:

$$Q \approx f(S^n) + f'(S^n)(S^{n+1} - S^n)$$

Where $S^n$ is the water storage at the start of the timestep, S^{n+1} the storage at the end of the time step and $f'(S^n)$ gives the sensitivity of flow to storage changes around the operating point.
