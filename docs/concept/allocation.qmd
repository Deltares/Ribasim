...
In Ribasim we have LinearResistance and ManningResistance. If $r \in S$ is such a resistance node and $l_r \in E_S$ is the inflow link of that resistance node, we add the decision variables $\mathcal{F}_{l_r}^+ \ge 0$, $\mathcal{F}_{l_r}^- \ge 0$ and the constraint

$$
\mathcal{F}_{l_r} = \mathcal{F}_{l_r}^+ - \mathcal{F}_{l_r}^-.
$$

That is, we split $\mathcal{F}_{l_r}$ into a positive and negative flow term. Furthermore we add a Boolean decision variable $\mathcal{B}_r$ and constraints

$$
    \mathcal{F}_{l_r}^+ \le F_\text{max}\mathcal{B}_r, \quad \mathcal{F}_{l_r}^- \le F_\text{max}(1-\mathcal{B}_r)
$$

to make sure that only one of the positive and negative flow variable can be non-zero. Now consider the following plot of the upstream level $h_a$ and downstream level $h_b$ space.
