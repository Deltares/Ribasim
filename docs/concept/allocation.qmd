...
For technical implementation details including how mathematical formulations translate to code, see the [Allocation](/dev/allocation.qmd).
:::
...
- Can only connect to the primary network via Pump or Outlet nodes, which must be allocation controlled
...
## Adaptive timestepping {#sec-adaptive-timestepping}

The allocation LP linearizes two types of relationships, both of which are only valid for small level changes $\Delta h$. By bounding the linearization errors, we can derive an adaptive timestep that keeps all approximations accurate while taking large steps when possible.

### Basin profile linearization error

The basin profile has a piecewise-linear area $A(h)$, so the storage $S(h) = \int_0^h A(h')\,dh'$ is piecewise quadratic. Within a profile segment where the area varies linearly with slope $m = \frac{dA}{dh}$:

$$A(h) = A^n + m\,(h - h^n)$$

The linearization approximates the storage with a tangent line:

$$S^*(h) = S^n + A^n\,(h - h^n)$$

The true storage is obtained by integrating the area:

$$S(h) = S^n + \int_{h^n}^{h} A(h')\,dh' = S^n + A^n\,(h - h^n) + \tfrac{1}{2}\,m\,(h - h^n)^2$$

so the linearization error is

$$|S(h) - S^*(h)| = \tfrac{1}{2}|m|\,(\Delta h)^2$$

Setting this to be at most a fraction $\varepsilon$ of the maximum basin storage capacity $S_{\max}$:

$$\tfrac{1}{2}|m|\,(\Delta h)^2 \leq \varepsilon\, S_{\max}$$

gives a maximum level change

$$\Delta h_{\max}^{\text{basin}} = \sqrt{\frac{2\,\varepsilon\, S_{\max}}{|m|}}$$

When $|m| \approx 0$ (basin with constant area), the linearization is exact and imposes no constraint.

### Connector node linearization error

Connector nodes ([LinearResistance](/reference/node/linear-resistance.qmd), [ManningResistance](/reference/node/manning-resistance.qmd) and [TabulatedRatingCurve](/reference/node/tabulated-rating-curve.qmd)) have nonlinear flow-level relationships $Q(h)$ that are also linearized:

$$Q^{n+1} \approx Q^{n} + \frac{\partial Q}{\partial h}(h^n)\,(h^{n+1} - h^{n})$$

The second-order error of this Taylor expansion is

$$|Q(h) - Q^*(h)| = \tfrac{1}{2}\left|\frac{d^2 Q}{d h^2}\right|\,(\Delta h)^2$$

Bounding this by a flow error tolerance $\varepsilon_Q$:

$$\Delta h_{\max}^{\text{connector}} = \sqrt{\frac{2\,\varepsilon_Q}{\left|\frac{d^2 Q}{d h^2}\right|}}$$

The second derivative $\frac{d^2 Q}{d h^2}$ is computed numerically via nested automatic differentiation. This handles all connector types uniformly, including the PCHIP interpolation of tabulated rating curves and the nonlinear Manning equation.

### Global maximum level change

The adaptive timestep is governed by whichever linearization is most sensitive to level changes:

$$\Delta h_{\max} = \min\left(\min_i \Delta h_{\max,i}^{\text{basin}},\; \min_j \Delta h_{\max,j}^{\text{connector}}\right)$$

### Adaptive timestep

Converting the level change bound to a time bound using $\Delta S \approx A^n \,\Delta h$ and the current net storage rate $\dot{S}$:

$$\Delta t_i = \frac{A_i^n \cdot \Delta h_{\max}}{|\dot{S}_i|}$$

The adaptive timestep for the subnetwork is:

$$\Delta t_{\text{adaptive}} = \max\left(\min_i \Delta t_i,\; \Delta t_{\min}\right)$$

bounded below by the configured minimum `dtmin`, and above by the time to the next save point or the end of the simulation.
...
The allocation algorithm optimizes for a sequence of objectives in succession. Before optimizing the next goal, it adds an inequality constraint that retains the preceding result in the direction of improvement. For a minimization objective with optimal value $z^*$, this is $z \le z^*$. This approach is known as [goal programming](https://en.wikipedia.org/wiki/Goal_programming).
...
As indicated, each of these allocated flow variables is bounded between 0 and the corresponding demand (NOTE: in this text we assume that the demands are already expressed in the scaled flow unit). A UserDemand node can have multiple inflow links; for UserDemand nodes we have that the sum of the flows over all inflow links is equal to the sum of the allocated flow variables over the priorities:

$$
	\sum_{b \in \text{in}(v)} F_{(b, v)} = \sum_{p \in P_v} F^p_v, \quad \forall v
	\in UD.
$$
...
$$
	0 \le F^0_v \le \max\left(0, Q^\max_{(b_\text{upstream}, v)}\right), \quad \forall v \in FD
$$

where $Q^\max_{(b_\text{upstream}, v)}$ is the upper flow capacity of the inflow link of $v$ (see @sec-flow-capacities).

We only let $F^0_v$ account for surplus flow, because if we allow that variable to be negative it can be used to allocate to demands. To make sure we still account for the possibility that the flow through the node with the flow demand is negative, we allow the allocated flow variable with the earliest priority to be negative down to the lower flow capacity $\min\left(0, Q^\min_{(b_\text{upstream}, v)}\right)$ of that link, and account for this when writing the results.
...
Now, just as in the previous section, we add a subsequent objective for fair distribution. LevelDemand errors are storage volumes, whereas fairness must compare the corresponding level errors. For every priority $p$ and side $s \in \{\text{lower}, \text{upper}\}$, let $B_{p,s}$ be the participating Basins and let $n_{p,s} = |B_{p,s}|$. We calculate the current mean Basin area

$$
\bar A_{p,s} = \frac{1}{n_{p,s}} \sum_{b \in B_{p,s}} A_b,
$$

and the mean storage error

$$
\bar E_{p,s} = \frac{1}{n_{p,s}} \sum_{b \in B_{p,s}} E^p_{b,s}.
$$

The common area scale $\bar A_{p,s}$ converts every storage error to a proportionally scaled level error. It does not change the ordering of the fairness objective, while keeping LP coefficients close to one even for large Basin areas.
...
$$
	\overline{E}^p_{b, \text{lower}} \ge \frac{\bar A_{p,\text{lower}}}{A_b} E^p_{b, \text{lower}} - \bar E_{p,\text{lower}}, \quad \forall b \in B_{p,\text{lower}};
$$

$$
	\overline{E}^p_{b, \text{upper}} \ge \frac{\bar A_{p,\text{upper}}}{A_b} E^p_{b, \text{upper}} - \bar E_{p,\text{upper}}, \quad \forall b \in B_{p,\text{upper}},
$$
...
When Pumps and Outlets are part of a subnetwork, they can be controlled by allocation. To accomplish this, they must have `allocation_controlled` set to `true` in the static table for [Pump](/reference/node/pump.qmd#static) or [Outlet](/reference/node/outlet.qmd#static). When a Pump or Outlet is allocation controlled, the flow through the node is only bounded by its capacity and Pump or Outlet specific constraints based on the difference between upstream and downstream level (see [Pump equations](/reference/node/pump.qmd#equations) or [Outlet equations](/reference/node/outlet.qmd#equations)).

:::{.callout-important}
Because the allocation problem must be bounded, an allocation controlled Pump or Outlet is required to have a finite `max_flow_rate`. This is validated at the start of the simulation, for the initial parameters as well as for all DiscreteControl states of the node. Note that Pumps and Outlets that connect the primary network to a secondary network are required to be allocation controlled; an error is thrown if `allocation_controlled` is not set to `true` for these nodes. Hence they also require a finite `max_flow_rate`.
:::

For a TabulatedRatingCurve, the allocation-determined flow is bounded by the maximum flow the rating curve can physically deliver at the highest upstream level reachable within the allocation time step, i.e. $Q(h)$ evaluated at that level. In the physical layer, the allocated flow rate replaces the usual $Q(h)$ lookup while still applying the same reduction factors for low storage, level difference, and maximum downstream level (see [TabulatedRatingCurve equations](/reference/node/tabulated-rating-curve.qmd#equations)). After all goals have been optimized for, the flow rate through the allocation controllable node is communicated to the physical layer.
...
The allocation problem is solved per subnetwork, which is given by a subset $S \subset V$ of node IDs. Different subnetworks are disjoint from each other. Nodes can also not be part of any subnetwork. The flow links of the problem are all links that have at least one of their two nodes in $S$, so links that connect the subnetwork to the primary network are included.

### Boundary flows

Links pointing out of a FlowBoundary node carry a known flow rate rather than a decision variable: the corresponding flow variable is fixed to the average flow over the last allocation interval $\Delta t_{\text{alloc}}$ in the physical layer,

$$
    \frac{1}{\Delta t_{\text{alloc}}}\int_{t - \Delta t_{\text{alloc}}}^tQ_{ij}(t') dt'.
$$

For the definition of $Q_{ij}$ see [the formal model description](/concept/equations.qmd#formal-model-description). LevelBoundary nodes instead enter the problem through their level, which is a fixed parameter of the optimization (see [Boundary nodes](#boundary-nodes)).
...
UserDemand nodes dictate proportional relationships between flows over links in the subnetwork. The return factor is given by $0 \le r_i(t) \le 1, i \in U_S$: the outflow of a UserDemand node is $r_i$ times the sum of the flows over all its inflow links.

## Flow capacities {#sec-flow-capacities}

Every flow variable in the optimization problem needs bounds, both because unbounded variables make the problem numerically poorly behaved and because an unbounded problem cannot be solved at all. These bounds are recomputed before every optimization, because they depend on the current state of the physical layer and on the allocation timestep $\Delta t$.

For each flow link, the bounds are the intersection of the bounds imposed by the node at either end of the link. Per node type:

- **FlowBoundary**: the flow variable is fixed to the prescribed boundary flow, so no bounds are needed.
- **Pump and Outlet**: bounded by `min_flow_rate` and `max_flow_rate`. Since allocation assigns a single flow rate for the whole timestep, the tightest bounds over $[t, t + \Delta t]$ are used, i.e. the largest minimum and the smallest maximum of the two interpolated endpoints.
- **UserDemand**: both the inflow links and the return flow link are bounded between $0$ and the total demand of the node summed over all its demand priorities. For the upper bound the return factor is conservatively assumed to be $1$.
- **LinearResistance, ManningResistance and non-allocation-controlled TabulatedRatingCurve**: the flow of these nodes is given by the linearized relation

  $$Q \approx Q^n + \frac{\partial Q}{\partial h_a}\Delta h_a + \frac{\partial Q}{\partial h_b}\Delta h_b.$$

  Evaluating this expression over the interval of level changes $\Delta h$ that the Basins at either end can reach within the timestep gives the tightest bounds consistent with the flow constraint itself. The reachable level change of a Basin follows from the bounds on its storage change decision variable $\Delta S$ using the same linearized profile the constraints use, $\Delta h = \Delta S / A^n$. Levels of LevelBoundary and Terminal nodes are fixed within one optimization, so they contribute no level change.
- **Allocation controlled TabulatedRatingCurve**: bounded between $0$ and $Q(h_{a,\max})$, where $h_{a,\max}$ is the highest upstream level reachable within the timestep. Because $Q(h)$ is validated to be non-decreasing and all reduction factors applied in the physical layer are in $[0, 1]$, this is a valid upper bound.
- **Allocation controlled Pump and Outlet**: bounded by `max_flow_rate`, which is [required to be finite](#control-by-allocation).

Because a bound derived this way follows directly from the flow constraint of the node, it can never make the problem infeasible: any flow that satisfies the constraints also satisfies the bound.

## Route priority

Nodes can be assigned priorities that influence how water is preferentially routed. These priorities are given as weights $w_i$ for node $i$ with an outflow link $Q_i$.
...
Both positive and negative route priorities are supported. A negative route priority turns the minimization to a maximization of flow through that node.

For how route priorities are configured in model settings, see [Usage: Allocation settings](/reference/usage.qmd#sec-allocation-settings).
