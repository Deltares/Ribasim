...
When we build a Ribasim model in which we want to uttilize allocation, it can be computationally efficient and stable to divide a large allocation network into a primary network which connects to multiple subnetworks. The primary network should be the main water system and treats each connected subnetwork as a single demand node.
...
3. Perform sequentially allocation (or in the future parallel) in the subnetworks.
...
with $\frac{dh}{dS}(S^n) = \frac{1}{A^n}$ and $A^n$ is the area evaluated at the beginning of the time step
...
$$ Q_m^{n+1} \approx Q_m^{n} + \frac{\partial Q_m}{\partial h_a}^n(h_a^{n+1} - h_a^{n}) + \frac{\partial Q_m}{\partial h_b}^n(h_b^{n+1} - h_b^{n}) $$
...
- The Terminal, where water can simply leave the model
- The LevelBoundary, which yields fixed water levels; $h^{n+1}_{lb} $ read from the interpolated timeseries $h_{fb}$ of the node or is a constant value
