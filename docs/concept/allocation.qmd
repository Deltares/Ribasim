...
To use efficient linear programming algorithms, nonlinear relationships can be linearized around the current time $t^n$. For example, a nonlinear relationship between flow $Q$, an upstream  variable ($S_1$) and a downstream variable ($S_2$), can be linearized as:
...
In Ribasim, connector nodes can determine the flow as function of upstream and/or downstream level/storage. These are [LinearResistance](../reference/node/linear-resistance.qmd), [ManningResistance](../reference/node/manning-resistance.qmd) and [TabulatedRatingCurve](../reference/node/tabulated-rating-curve.qmd) nodes. The linear resistance node is linear in level but not in storage. However, we can directly substitute the linearized level-storage relationship into
...
$$ Q_m^{n+1} = Q_m^{n} + \frac{\partial Q_m}{\partial S_a}(S_1^n,S_2^n)(S_a^{n+1} - S_a^{n}) + \frac{\partial Q_m}{\partial S_b}(S_1^n,S_2^n)(S_b^{n+1} - S_b^{n}) $$
...
$$ Q_m^{n+1} = Q_m^{n} + \frac{\partial Q_m}{\partial h_a}(h_a^n, h_b^n) \frac{1}{A_a^n}(S_a^{n+1} - S_a^{n}) + \frac{\partial Q_m}{\partial h_b}(h_a^n, h_b^n) \frac{1}{A_b^n} (S_b^{n+1} - S_b^{n}) $$

However, some nodes in Ribasim calculate flows in terms of level, for which the level is not reducible to a storage. $$Q(S_1,S_2) \approx Q(S_1^n,S_2^n) + \frac{\partial Q}{\partial S_1}(S_1^n,S_2^n)(S_1^{n+1}-S_1^n) + \frac{\partial Q}{\partial S_2}(S_1^n,S_2^n)(S_2^{n+1}-S_2^n)$$
