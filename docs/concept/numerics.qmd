...
As the cumulative flows in $\mathbf{u}$ grow over time, the allowed error tolerance based on `reltol` grows as well. After a thousand seconds with a flow of $1\text{ m}^3/\text{s}$, the error tolerance has decreased from $1^{-5}$ to $1^{-2}$. To compensate for this, we decrease the `reltol` individually per state over time.

The formula for the new tolerance can be expressed as:
...
- $\text{reltol}$ is the configured relative tolerance.
...
This formula ensures that the new tolerance decreases based on the difference between the cumulative magnitude and the average magnitude (once the cumulative magnitude exceeds $10^{4}$), with a lower bound of $10^{-14}$ to maintain numerical precision.
