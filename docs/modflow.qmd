...
Ribasim connects to MODFLOW6 via the Basic Model Interface (BMI) and Extended
...
MODFLOW6 may compute a infiltration flux. Ribasim then runs its timestep, and
...
information: which MODFLOW6 boundary condition is connected to which Ribasim
...
the volumes are converted to MODFLOW6 boundary condition levels using a
Callback function.
...
coupled solution, we are solving both MODFLOW6 and Ribasim repeatedly, until they
...
In coupling Ribasim to MODFLOW6, relations translating the Ribasim volume must be
...
coordinates must match the domain of the coupled MODFLOW6 model exactly. 
...
coupled MODFLOW6 boundary condition, describing the volume-level lookup table
per cell.

:::{.callout-note}
The `x` and `y` coordinates are valid for structured MODFLOW6 models (DIS).
...
connected to multiple MODFLOW6 cells, and the coupling parameters must match
the (structured, unstructured) grid of the MODFLOW6 model exactly.
:::

The MODFLOW6 coupling example cases show examples of such a parametrization.
