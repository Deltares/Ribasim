...
To run this tutorial locally, install the latest release of Ribasim as documented in [the installation guide](/getting-started/install.qmd).
...
Read the result files and plot the simulated flows from different links and the levels and storages at our confluence point:

```{python}
ds_basin = xr.open_dataset(base_dir / "Crystal-1/results/basin.nc")
...
ds_flow = xr.open_dataset(base_dir / "Crystal-1/results/flow.nc")
