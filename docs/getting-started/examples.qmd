...
The TabulatedRatingCurve has `control_state="Ribasim.allocation"` and a negative `route_priority`, so under normal conditions it follows the physical $Q(h)$ curve, but allocation reduces the flow when the Basin level drops toward the minimum level demand.
