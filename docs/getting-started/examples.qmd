...
ds_basin = xr.open_dataset(datadir / "basic/results/basin.nc")
...
```{python}
ds_flow = xr.open_dataset(datadir / "basic/results/flow.nc")
...
- If the level rises above 15 m, activate the control state "out" and retain it until the level falls below 14 m;
- If the level falls below 4 m, activate the control state "in" and retain it until the level rises above 5 m;
...
```{python}
ds_basin = xr.open_dataset(datadir / "level_range/results/basin.nc")
df_basin = ds_basin.to_dataframe().reset_index()
df_basin_wide = df_basin.pivot_table(
    index="time", columns="node_id", values=["storage", "level"]
)
...
thresholds = model.discrete_control.condition.df
threshold_high = thresholds.threshold_high.to_numpy()
threshold_low = thresholds.threshold_low.to_numpy()
...
ax.set_yticks([4.0, 5.0, 14.0, 15.0], ["start in", "stop in", "stop out", "start out"])
ax.set_ylabel("level")
plt.show()
```

We see that in January the Basin level is too high, so water is pumped out until the level falls below 14 m. Then, until May, water drains freely through the TabulatedRatingCurve until the level drops below 4 m. From this point until the start of July, water is pumped into the Basin in short bursts: each burst continues until the level rises above 5 m. At the start of July, rain starts falling on the Basin, causing its level to rise. Once it rises above 15 m, water is pumped out in short bursts until the level falls below 14 m.
...
```{python}
model = Model(starttime="2020-01-01", endtime="2021-12-01", crs="EPSG:4326")
```
...
model.level_boundary.add(
    ...
    [level_boundary.Static(level=[5.5])],
)
```
...
ds_basin = xr.open_dataset(datadir / "pid_control/results/basin.nc")
df_basin = ds_basin.to_dataframe().reset_index()

fig, ax = plt.subplots()
ax.plot(df_basin.time, df_basin.level, label="level")
ax.set_ylabel("level [m]")

# Plot target level of the pump PID controller
pid_pump = model.pid_control.time.df[model.pid_control.time.df.node_id == 5]
level_demands = pid_pump.target.to_numpy()
times = date2num(pid_pump.time)
ax.step(times, level_demands, where="post", color="k", ls=":", label="target")
ax.legend()
pass
```
...
Setup outlets with allocation control. Note `allocation_controlled=True` which tells the allocation algorithm to control these nodes:
...
```{python}
ds_basin = xr.open_dataset(datadir / "polder_management/results/basin.nc")
df_basin = ds_basin.to_dataframe().reset_index()
df_basin_wide = df_basin.pivot_table(
    index="time", columns="node_id", values=["storage", "level"]
)
...
```{python}
ds_flow = xr.open_dataset(datadir / "polder_management/results/flow.nc")
df_flow = ds_flow.to_dataframe().reset_index()
df_flow["link"] = list(zip(df_flow.from_node_id, df_flow.to_node_id))
...
```{python}
ds_flow = xr.open_dataset(datadir / "outlet_continuous_control/results/flow.nc")
df_flow = ds_flow.to_dataframe().reset_index()
fig, ax = plt.subplots()
...
# Allocation-controlled TabulatedRatingCurve {#allocation-controlled-tabulatedratingcurve}

This example demonstrates how a [TabulatedRatingCurve](/reference/node/tabulated-rating-curve.qmd) can be controlled by [allocation](/concept/allocation.qmd).
A Basin drains through a TabulatedRatingCurve into a Terminal.
A LevelDemand on the Basin tells the allocation algorithm to maintain the Basin level between 3 and 4 m.
The TabulatedRatingCurve is allocation controlled and has a negative `route_priority`, so under normal conditions it follows the physical $Q(h)$ curve, but allocation reduces the flow when the Basin level drops toward the minimum level demand.
...
Write the model to a temporary directory and run it:

```{python}
datadir = Path("data/level_demand_with_rating_curve")
model.write(datadir / "ribasim.toml")
result = run_ribasim(datadir / "ribasim.toml")
```

Read back and plot the Basin level over time, including the minimum level demand:

```{python}
import pandas as pd
import matplotlib.pyplot as plt

ds_basin = xr.open_dataset(datadir / "results/basin.nc")
df_basin = ds_basin.to_dataframe().reset_index()
df_basin_1 = df_basin[df_basin["node_id"] == 1]

fig, ax = plt.subplots()
df_basin_1.plot(x="time", y="level", ax=ax, label="Basin #1 level")
ax.axhline(y=3.0, color="red", linestyle="--", label="min_level = 3.0 m")
ax.set_ylabel("level [m]")
ax.legend();
```

Plot the flow through the TabulatedRatingCurve:

```{python}
ds_flow = xr.open_dataset(datadir / "results/flow.nc")
df_flow = ds_flow.to_dataframe().reset_index()
df_flow_trc = df_flow[
    (df_flow["from_node_id"] == 1) & (df_flow["to_node_id"] == 2)
]

fig, ax = plt.subplots()
df_flow_trc.plot(x="time", y="flow_rate", ax=ax, label="Basin #1 → TRC #2")
ax.set_ylabel("flow [m³s⁻¹]")
ax.legend();
```

The plot shows that the TabulatedRatingCurve initially follows its $Q(h)$ relationship.
As the Basin level drops toward the 3.0 m minimum level demand, allocation reduces the outflow to zero, maintaining the target level.
