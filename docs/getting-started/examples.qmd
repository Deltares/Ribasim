...
All nodes need to be assigned to a subnetwork for allocation:
...
# Polder management with discrete control (alternative approach)

**Comparison**: While the previous example used allocation for water management, this example shows how to achieve similar water management goals using traditional discrete control with hysteresis. This approach can be useful when:

- You need explicit control over switching behavior
- You want to implement custom control logic

The key differences:

- **Discrete control** instead of allocation for pumps and outlets
- **Hysteresis logic** to prevent rapid switching (different on/off thresholds)
- **No level demand nodes** - target levels are embedded in discrete control logic
- **Flow demands** still work but with regular discrete control

Setup the model without allocation:

```{python}
model = Model(starttime="2020-01-01", endtime="2021-01-01", crs="EPSG:4326")
```

Use the same precipitation and evaporation patterns:
...
Setup outlets with different control strategies:

```{python}
# Inlet with discrete control - opens when polder 6 is low
outlet5 = model.outlet.add(
    Node(5, Point(2, 1), name="inlaat"),
    [
        outlet.Static(
            ...
            max_downstream_level=[1.00, 1.02],
        )
    ],
)

# Simple flow-controlled outlet for water transfer between polders
outlet13 = model.outlet.add(
    Node(13, Point(3, 2), name="inlaat/uitlaat"),
    [
        outlet.Static(
            flow_rate=5.0, min_upstream_level=[0.98], max_downstream_level=[0.95]
        )
    ],
)

# Uncontrolled outlets for drainage
outlet10 = model.outlet.add(
    Node(10, Point(5.0, 0.0)),
...
Setup discrete control for the inlet - controls based on polder 6 level:

```{python}
dc_inlaat5 = model.discrete_control.add(
    Node(50, Point(2.3, 1.2), name="dc_inlaat5_op_peil6"),
    [
        # Listen to polder 6 level
        discrete_control.Variable(
            compound_variable_id=1,
            listen_node_id=6,
            variable=["level"],
        ),
        # Open only when level drops below 0.9 (hysteresis band 0.90/0.95)
        discrete_control.Condition(
            compound_variable_id=1,
            condition_id=[1],
            threshold_high=[0.95],  # True if > 0.95
            threshold_low=[0.9],     # False if ≤ 0.9
        ),
        # T (>0.95) -> closed, F (≤0.9) -> open
        discrete_control.Logic(
            truth_state=["T", "F"],
            control_state=["closed", "open"],
        ),
...
model.link.add(dc_inlaat5, outlet5)
```

Setup pump with discrete control using hysteresis:

```{python}
# Pump with hysteresis control
pump7 = model.pump.add(
    ...
    [
        pump.Static(
            ...
            flow_rate=[0.0, 5.0],
            min_upstream_level=[0.90, 0.91],
        )
    ],
)

dc_pomp7 = model.discrete_control.add(
    Node(20, Point(4.5, 1.5), name="dc_pomp7"),
    [
        # Listen to polder 6 level
        discrete_control.Variable(
            compound_variable_id=1,
        ...
        # ON at >0.95, OFF at ≤0.90 (hysteresis prevents rapid switching)
        discrete_control.Condition(
            compound_variable_id=1,
            condition_id=[1],
            threshold_high=[0.95],
...
model.link.add(dc_pomp7, pump7)
```

Add flow demand for flushing (works with regular control):
...
pump7_flow_demand = model.flow_demand.add(
...
model.link.add(pump7_flow_demand, pump7)
```
...
```{python}
model.write(datadir / "polder_management_discrete/ribasim.toml")
run_ribasim(datadir / "polder_management_discrete/ribasim.toml")
```

Analyze the results:

```{python}
df_basin = pd.read_feather(datadir / "polder_management_discrete/results/basin.arrow")
df_basin_wide = df_basin.pivot_table(
    index="time", columns="node_id", values=["storage", "level"]
)
...
Examine the flow rates through key structures:

```{python}
df_flow = pd.read_feather(datadir / "polder_management_discrete/results/flow.arrow")
df_flow["link"] = list(zip(df_flow.from_node_id, df_flow.to_node_id))
df_flow["flow_m3d"] = df_flow.flow_rate

# Focus on controlled structures
controlled_links = [(3, 5), (4, 13), (6, 7)]  # inlet, polder connection, pump
df_controlled = df_flow[df_flow["link"].isin(controlled_links)]
df_pivot = df_controlled.pivot_table(index="time", columns="link", values="flow_m3d")
