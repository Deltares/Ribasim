...
The core of Ribasim supports [GeoPackage](https://www.geopackage.org/) as its input data structure.
And so does [QGIS](https://qgis.org/).
This makes QGIS the perfect application to create, edit, and inspect Ribasim models.
The Ribasim QGIS plugin aids users in setting up the model by providing valid values while editing.
Deltares also provides the iMOD QGIS plugin, which can be used to inspect the output results of a Ribasim model too.
