---
title: "Bach-Modflow Demonstration"
---
...
An example of the MODFLOW6 section of TOML configuration required for a coupled
...
The section starts by stating the path to the MODFLOW6 simulation name file
(`simulation`). The next section contains the information regarding the
MODFLOW6 model(s) to couple to Bach's basins. Per model, a path to the coupling
...
that should be coupled to Bach, the package names (`river`, `drain`) used by
MODFLOW6 must be specified (as we look for these names in the MODFLOW6 memory),
...
2. A coupled run where the MODFLOW6 stages are updated by the Bach coupler
process, but without Bach. This results in volumes of 0.0, so all MODFLOW6
water levels are set equal to bed elevation.
3. A coupled run where the water levels are updated by Bach.
...
![Water balance of the MODFLOW6 boundary conditions for the Hupsel basin for a standalone MODFLOW6 run. The four sequential steady states (01, 02, 03, 04) use net groundwater recharge values of 1.0, 0.5, -0.05 and -0.1 mm/d.](https://user-images.githubusercontent.com/13662783/187715341-fd99dc06-2eda-4b84-a201-650dc7220574.png){#fig-hupsel-gwb-steady}
...
With evapotranspiration excess (-0.05 and 0.1 mm/d; low values are chosen here
...
![Water balance of the MODFLOW6 boundary conditions for the Hupsel basin for a zero volume run. The four sequential steady states (01, 02, 03, 04) use net groundwater recharge values of 1.0, 0.5, -0.05 and -0.1 mm/d.](https://user-images.githubusercontent.com/13662783/187715349-9eee8830-9b03-4a88-b7c9-b16004c694f6.png){#fig-hupsel-gwb-volume0}

@fig-hupsel-gwb-volume0 shows the same model, with 0-basin volume, and
elevations set equal to bed elevation. Consequently, primary and secondary
outflow terms are larger for positive groundwater recharge as they drain at a
lower level and intercept the water before the tertiary system does. Secondly,
with negative groundwater recharge, no infiltration occurs and the water is
drawn from the surroundings instead. This shows the coupling mechanism
adjusting MODFLOW6 water levels succesfully.

:::{.callout-note}
Add the Bach run with sufficient timesteps
:::
...
![Water balance of the MODFLOW6 boundary conditions for De Tol basin for a standalone MODFLOW6 run. The four sequential steady states (01, 02, 03, 04) use net groundwater recharge values of 1.0, 0.5, -0.05 and -0.1 mm/d.](https://user-images.githubusercontent.com/13662783/187720891-798f1f24-8664-492d-a21f-365ed1085ed8.png){#fig-tol-gwb-steady}
