...
In `set_objective_priority`, it sets the objective function for a given priority with the following steps:
...
- Basin capacities
- Buffer capacities
- User demands fractional return flow
...
## Output data
Function `save_demands_and_allocations` saves the demand and the allocated value per demand node. And function `save_allocation_flows` saves the optimized flows over the edges in the subnetwork.
