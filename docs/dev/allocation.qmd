...
average_flow_unit_error_constraint =
    @constraint(
        ...
        average_flow_unit_error[demand_priority] ==
            sum(weight * error[node_id, demand_priority, 1]
...
struct ScalingFactors
    ...
    mean_half_storage::Float64  # Initialization-time reference storage (m³)
end
...
function update_scaling!(p::Parameters, du::CVector, t::Float64, Δt::Float64)
    ...
    for allocation_model in allocation.allocation_models
        ...
        (; basin_ids_subnetwork) = node_ids_in_subnetwork
        mean_storage = sum(current_storage[id.idx] for id in basin_ids_subnetwork) /
            length(basin_ids_subnetwork)
        scaling.storage = (scaling.mean_half_storage + mean_storage) / 2
...
- The flow scale equals `scaling.storage / Δt`, so flow coefficients in the discretized volume-conservation equations are one
...
struct AllocationModel
    ...
    cumulative_supplied_volume::Dict{Tuple{NodeID, NodeID}, Float64}
    ...
    level_demand_area_sum::Dict{Int32, Float64}
    level_demand_count::Dict{Int32, Int}
    level_demand_area_scale::Dict{Int32, Float64}
    ...
    for node_id in user_demand_ids_subnetwork
        for (demand_priority_idx, demand_priority) in enumerate(demand_priorities_all)
            ...
            record_demand[subnetwork_id][demand_priority][node_id] = (
                ...
                supplied = 0.0  # To be filled in after physical layer runs
            )
        end
    end
end
...
When a Pump or Outlet has `allocation_controlled` set to `true`:
