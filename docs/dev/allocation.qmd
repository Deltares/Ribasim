...
function update_scaling!(p::Parameters, du::CVector, t::Float64, Δt::Float64)
    ...
    for allocation_model in allocation.allocation_models
        (; node_ids_in_subnetwork, scaling, flow_links_subnetwork) = allocation_model
        (; basin_ids_subnetwork) = node_ids_in_subnetwork
        ...
        mean_flow = sum(
            abs(get_flow(du, p_independent, t, link)) for link in flow_links_subnetwork
        ) / length(flow_links_subnetwork)
        scaling.flow = (scaling.storage / Δt + mean_flow) / 2
    end
end
```
...
- The flow scale is the average of the storage-derived flow scale and the mean magnitude of the current physical flows in the subnetwork
