...
In function `set_initial_values` capacities and demands are initialized.
```julia
...
```
Within the subnetwork, capacities of `source` is set to the average flow in the past allocation period. It updates `source` constrains.

Initial capacities for all edge
