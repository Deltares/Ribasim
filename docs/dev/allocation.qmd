...
[AllocationModel struct](https://deltares.github.io/Ribasim/dev/allocation.html#the-allocation-struct) is used for constructing JuMP problem.
When a Allocation model is created, a JuMP optimization model is defined and initialized in the allocation model.
More details on how allocation interacts with `JuMP` is explained [here](https://deltares.github.io/Ribasim/dev/allocation.html#the-allocation-struct).
...
The update of allocation are spread in three parts.
...
The mean flows data is used for iMOD coupler.
...
When working with optimization problems using JuMP, there are three fundamental components:
...
When [looping over priorities](https://deltares.github.io/Ribasim/dev/allocation.html#updating-capacities), the constraints are updated in the function `adjust_...`.

- Objective function: This is the function that is optimized in the optimization problem.
