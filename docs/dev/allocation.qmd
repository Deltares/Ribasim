...
- ["Allocate"](https://deltares.github.io/Ribasim/concept/allocation.html#the-high-level-algorithm). This step solves the optimization problems that allocates the demand to the subnetworks.
...
In `set_objective_priority`, it sets the objective function for a given priority with following steps:
...
In the function `set_initial_values`, following capacities and demands are initialized:
...
- Basin capacities 
- Buffer capacities
- User demands fractional retrun flow 
...
## Output data
Functions `save_demands_and_allocations` saves the demand and the allocated value per demand node. And `save_allocation_flows` save the optimized flows over the edges in the subnetwork.
These values are saved in the `record_demand` and `record_flow` fields of the `Allocation` struct and only writen to the output file at the end of the simulation.
