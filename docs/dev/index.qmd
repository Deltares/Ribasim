...
```sh
pixi run --environment=qgis install
```
...
You can do this in a terminal by calling `pixi shell`, or starting programs with `pixi run --environment=dev julia`, or `pixi run qgis`.
