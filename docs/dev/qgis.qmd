...
`pixi run python utils/node_icons.py`

The `utils/node_icons.py` script:
...
When changing icons, rerun `utils/node_icons.py` and include the resulting QML and icon changes in your PR.

You can also modify the QML files by making changes in QGIS.
For `NodeStyle.qml` it is best to run `node_icons.py` afterwards so the formatting stays the same.
