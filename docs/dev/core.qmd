...
The computational core is one of the components of Ribasim as illustrated in the [component overview](/getting-started/install.qmd#sec-components).
...
Click the green play icon in front of a test item, as shown in the image below.
...
To preview without executing code cells (much faster for checking layout):

```bash
pixi run docs -- --no-execute
```

To preview a single file:

```bash
pixi run docs guide/delwaq.qmd
```
