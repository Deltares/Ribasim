...
| `current_basin_properties` | Cached values that depend on both state and time (current level, storage, flow rates) | `Cache()` — copied as dual-number arrays |
...
jac_prep = prepare_jacobian(
    ...
    Cache(current_basin_properties),
