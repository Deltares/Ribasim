...
For pre-releases, add `-rcN` to the end of the version, which stands for release candidate.
...
Use find and replace to update the locations.
Don't yet change `install.qmd`, this happens in a later step.
...
Remember to save all files after edits.

::: {.callout-important}
Always update the example version numbers in `docs/guide/installing-other-versions.qmd` to refer to the latest release candidate and never to a stable release.
:::

::: {.callout-important}
The `Identity Version` in `build/AppxManifest.xml` consists of four numbers.
The fourth number should be reset to 1 when the 3-part version number changes.
When the 3-part version number does not change, increase it by one.
That means for the example of the two pre-releases of `2023.1.0` and its release, these should get 1, 2 and 3 respectively.
If the fourth number does not increase, users cannot update existing installations, but have to uninstall first.
:::
...
## Publish the Ribasim Python packages to PyPI
...
## Publish the QGIS plugin to the QGIS plugin repository

1) Download the `ribasim_qgis.zip` from the [latest GitHub release assets](https://github.com/Deltares/Ribasim/releases/latest).
2) Log in to the [QGIS plugin repository](https://plugins.qgis.org/).
3) Go to [Upload a plugin](https://plugins.qgis.org/plugins/upload/) and upload the zip file.
4) The plugin will be available after manual approval from a QGIS plugin repository maintainer.

## Update version numbers in docs

When the new release is published, we can update the version numbers in `install.qmd` as well,
so users are directed to the new release when installing.
