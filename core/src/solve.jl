...
water_balance!(
    ...
    p::Parameters,
    t::Number,
)::Nothing = water_balance!(du, u, p.p_non_diff, p.diff_cache, p.p_mutable, t)
...
function formulate_pid_control!(
    ...
    for (i, _) in enumerate(node_id)
        ...
        du.integral[i] = diff_cache.error_pid_control[i]
...
function formulate_dstorage(
    ...
    (; inflow_ids, outflow_ids, vertical_flux) = p_non_diff.basin
